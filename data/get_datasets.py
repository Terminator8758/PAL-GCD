...
def get_datasets(dataset_name, train_transform, test_transform, args, return_propagate_loader=False):
    ...
    # note: datasets = {'labelled':xx, 'unlabelled':xx, 'val':xx, 'test':xx}
    ...
    if return_propagate_loader:
        # added to obtain the whole train set for SS-K-Means evaluation during train
        trainset_propagate_1 = deepcopy(datasets['train_labelled'])
        trainset_propagate_1.transform = test_transform
        trainset_propagate_2 = deepcopy(datasets['train_unlabelled'])
        trainset_propagate_2.transform = test_transform
        trainset_propagate = MergedDataset(labelled_dataset=trainset_propagate_1, unlabelled_dataset=trainset_propagate_2)
    else:
        trainset_propagate = None

    return train_dataset, test_dataset, unlabelled_train_examples_test, trainset_propagate, datasets
...
def get_class_splits(args):
    ...
    if args.old_class_ratio < 0.5:
        assert(args.old_class_ratio > 0)
        #print('args.train_classes= {}, args.unlabeled_classes= {}'.format(args.train_classes, args.unlabeled_classes))
        old_class_num = int(args.old_class_ratio * (len(args.train_classes)+len(args.unlabeled_classes)))
        temp_train_classes = args.train_classes.copy()  # the train classes by default is already set as half of all classes
        args.train_classes = args.train_classes[0: old_class_num]
        args.unlabeled_classes = np.concatenate((args.unlabeled_classes, temp_train_classes[old_class_num:]))
        #print('with old class ratio {}, args.train_classes= {}, args.unlabeled_classes= {}'.format(args.old_class_ratio, args.train_classes, args.unlabeled_classes))
